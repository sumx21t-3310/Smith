...
namespace NebusokuDev.ShooterWeaponSystem.Editor
{
    public class RecoilPatternGeneratorWizard : ScriptableWizard
    {
        ...
        private void OnWizardOtherButton() { _temp = CreateRecoilPattern(len); }
        ...
        IRecoilVector[] CreateRecoilPattern(int length)
        {
            ...
            for (int i = 1; i <= pattern.Length; i++)
            {
                ...
                pattern[i - 1] = new FixedRecoilVector { Value = new Vector2(horizontal, vertical) };
            }
